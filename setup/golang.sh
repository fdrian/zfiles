...
install_go(){
    ...
    wget https://go.dev/dl/${GO_VERSION}.linux-amd64.tar.gz $DEBUG_STD
    sudo tar -C /usr/local -xzf ${GO_VERSION}.linux-amd64.tar.gz $DEBUG_STD
    rm -rf $GO_VERSION*
...
if command -v go &>/dev/null; then
    ...
    GO_VERSION=$(curl -L -s "https://golang.org/VERSION?m=text" | head -n 1 | sed 's/go//') # Strip "go" for comparison) 

    # Version Comparison
    if [[ $CURRENT_GO_VERSION < $GO_VERSION ]]; then 
        echo "${YELLOW}A newer version of Go ($GO_VERSION) is available. Would you like to update? (y/n) "  
